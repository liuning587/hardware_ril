...
static int cmux_N1 = 512;//1509 -> 130 [ length(1B) + data(<=127B) + FCS(1B) + FLAG(1B) ]
#if 0
...
DataLeft data_left[GSM0710_MAX_CHANNELS];
...
/* flag, GSM0710_EA=1 C channel, frame type, length 1-2 */
    ...
    ssize_t c, cur, written;
    unsigned char tmp[2048]; //must larger than (cmux_N1 + 7)
...
/* length */
    ...
    pthread_mutex_unlock(&write_frame_lock);
        ...
        while (cur < c) {
            do {
                        struct pollfd pollfds[] = {{serial.fd, POLLOUT, 0}};
                        int ret;
                        do {
                        ...
                        if (pollfds[0].revents & POLLOUT) {
                            written = write (serial.fd, tmp + cur, c - cur);
                        } else {
                            written = -1; //POLLERR POLLHUP POLLNVAL 
                        }
            } while (written < 0 && errno == EINTR);

            if (written < 0) {
                LOGMUX(LOG_WARNING, "Couldn't write the whole postfix to the serial port for the virtual port %d. Wrote only %ld bytes, errno: %d (%s)",
                                channel, (long)c, errno, strerror(errno));
                pthread_mutex_unlock(&write_frame_lock);
                return 0;
            }
...
#if 0
        ql_system("rm /dev/chn");
                    if(mkdir("/dev/chn",S_IRWXU|S_IRWXG|S_IRWXO) != 0)
                    {
                        LOGMUX(LOG_DEBUG,"Create /dev/chn Dir Error : %d",errno);
                        return 1;
                    }
#endif
                    char link_name[12];
                    char command[20];
                    sprintf(link_name,"/dev/chn/%d",channel->id);
        sprintf(command,"rm -rf %s",link_name);
        ql_system(command);
    ...
    if (serial.state == MUX_STATE_MUXING)
        for (i=1;i<GSM0710_MAX_CHANNELS;i++)
            if (channellist[i].fd < 0) // is this channel free?
            {
                ...
                //joe for new
                LOGMUX(LOG_DEBUG,"PTS: %s",pts);
                //ql_system("rm -rf /dev/chn");
                if (access("/dev/chn", O_RDWR) && mkdir("/dev/chn",S_IRWXU|S_IRWXG|S_IRWXO) != 0)
                {
                    LOGMUX(LOG_ERR,"Create /dev/chn Dir Error : %d (%s)",errno, strerror(errno));
                    return 1;
                }
                else
                {
                    LOGMUX(LOG_DEBUG,"Create /dev/chn Dir Success");
                }
                char link_name[12];
                sprintf(link_name,"/dev/chn/%d",i);
                char command[20];
                sprintf(command,"rm -rf %s", link_name);
                ql_system(command);
                //if(symlink(pts,link_name) != 0)
...
//    buf->newdataready = 1; /*signal assembly thread that new buffer data is ready*/
    buf->newdataready++; /*signal assembly thread that new buffer data is ready*/
    printf("joe: newdataready {++} [ %d ]\n",buf->newdataready);
    pthread_mutex_unlock(&newdataready_lock);
    pthread_cond_signal(&newdataready_signal);
    printf("joe: give newdataready_signal\n");
#endif
    LOGMUX(LOG_DEBUG,"Leave");
    return length;
//}}}
...
/* Okay, we're ready to start analyzing the frame and filter out any escape char */
    while (gsm0710_buffer_length(buf) > 0)
    {
        ...
        if (buf->adv_length >= (int)sizeof(buf->adv_data)) /* frame data too much for buffer.. increase buffer size? */
        {
...
//received ack for a command
            ...
            {
                LOGMUX(LOG_DEBUG,"Channel:%d FC:%d",(frame->data[i] & 252) >> 2,frame->data[0]&0x2);
                LOGMUX(LOG_DEBUG,"\n\nGET ACK FOR MSC\n\n");
                //sleep(1);
            }
            else
...
//{{{
        ...
        {
//joe            pthread_cond_signal(&newdataready_signal);
            pthread_cond_signal(&bufferready_signal);
            printf("joe: give bufferready_signal\n");
        }
...
//data from logical channel
                ...
                if (channellist[frame->channel].fd > 0) {//reopening, discard the data
                    write_result = write(channellist[frame->channel].fd, frame->data, frame->length);
                } else {
                ...
                if (write_result >= 0)
                {
                    ...
                    if(write_result < frame->length)
                    {
                        ...
                        LOGMUX(LOG_WARNING,"Write pts error1\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$n\n");
                        //usleep(1000*1000);
                    }
                }
                else{
                    ...
                    {
                        send_modem_status(frame->channel,1);
                        fill_in_list(frame->channel,frame->data,frame->length);   
                        //MSC_flag |=  1 << (frame->channel - 1);
                        LOGMUX(LOG_WARNING,"Write pts error2\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$n\n");
                        //usleep(1000*1000);
                    }
#if 0
                    switch (errno){
...
//        while (!(buf->datacount > 0) || !buf->newdataready) /* true if no data available in buffer or no new data written since last extract_frames() call */
        while (!(buf->datacount > 0) || buf->newdataready <= 0) /* true if no data available in buffer or no new data written since last extract_frames() call */
        {
            LOGMUX(LOG_DEBUG,"joe: assemble_frame_thread put to sleep. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
            printf("joe: wait newdataready_signal\n");
            pthread_cond_wait(&newdataready_signal,&newdataready_lock); /* sleep until signalled by thread_serial_device_read() */
            LOGMUX(LOG_DEBUG,"joe: assemble_frame_thread awoken. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
            printf("joe: get newdataready_signal\n");
        }
//        buf->newdataready = 0; /*reset newdataready since buffer will be processed in extract_frames()*/
        buf->newdataready--; /*reset newdataready since buffer will be processed in extract_frames()*/
        printf("joe: newdataready {--} [ %d ]\n",buf->newdataready);
        pthread_mutex_unlock(&newdataready_lock);
...
//{{{
  ...
  while (1)
  {
    ...
    do {
        ret = poll(pollfds, 1, (MSC_flag != 0) ? 1000 : -1); //poll channle X to get POLLOUT is more better
    } while ((ret < 0 ) && (errno == EINTR));    
    ...
    ret = (*(poll_thread_arg->read_function_ptr))(poll_thread_arg->read_function_arg); /*Call reading function*/
//    if((*(poll_thread_arg->read_function_ptr))(poll_thread_arg->read_function_arg)!=0){ /*Call reading function*/
    if(ret == 1){ /*Call reading function*/
        LOGMUX(LOG_WARNING, "Device read function returned error");
//joe for test
        //extract_frames(serial_p->in_buf);
        goto terminate;
    }
    //joe for new
    else
    {
        if(gsm0710_buffer_free(serial_p->in_buf) <= (GSM0710_BUFFER_SIZE - 5))
        {
            LOGMUX(LOG_DEBUG, "Device read function returned success");
            extract_frames(serial_p->in_buf);
        }
    }
  }
...
//{{{
    ...
    //            LOGMUX(LOG_DEBUG, "Serial Data");
                unsigned int length;
//                if ((length = gsm0710_buffer_free(serial->in_buf)) > 0  && (serial->in_buf->received_count != 30) ) /*available space in buffer (not locked since we want to utilize all available space)*/
                ...
                {
      //              LOGMUX(LOG_DEBUG,"free length:%d",length);
                    if((len = read(serial->fd, buf, min(length, sizeof(buf)))) > 0)
                    {
                        LOGMUX(LOG_DEBUG,"READ SIZE : %d",len);
                        syslogdump("<s ", buf, len);
...
//                    while(1)
                    {
                        printf("++++++++++++++++++++++++++++++++++\n");
                        LOGMUX(LOG_DEBUG,"joe: ser_read_thread put to sleep. GSM0710 buffer has %d bytes free", gsm0710_buffer_free(serial->in_buf));
                        printf("joe: wait bufferready_signal\n");
                        pthread_cond_wait(&bufferready_signal,&bufferready_lock); /* sleep until signalled by assembly thread() */
                        LOGMUX(LOG_DEBUG,"joe: ser_read_thread awoken");
                        printf("joe: get bufferready_signal\n");
                    }
...
//{{{
    ...
    // modify by wythe 2013-12-12
#if 0
    SYSCHECK(chat(serial->fd, "AT+CFUN=4\r\n", 1));
#endif
...
#endif
    ql_cmux_debug = 1;
    LOGMUX(LOG_INFO, "Quectel CMUX Revision: Quectel_CMUX_SR01A05");
    LOGMUX(LOG_DEBUG, "Enter");
...
//Initialize modem and virtual ports
    ...
    LOGMUX(LOG_INFO,"\t-c <hardware flow control>:  Hardware flow control [%s]\n", hardware_flow_control?"enabled":"disabled");
...
int gsm0710muxd(const char *serialname, int speed, int ctsrts) {
    ...
    if (child_pid == 0) {
        ...
        do {
            sleep(1);
        } while (!qkill(child_pid, 0) && access("/dev/chn/3", O_RDWR));
        
            if (!qkill(child_pid, 0)) {
            if (!access("/dev/chn/3", O_RDWR)) {
